...
const startReactionListener = () => {
  ...
  console.log(
    `Escucha de reacciones activada desde: ${new Date(
      startTime
    ).toLocaleString()}`
  );
};
  ...
  try {
    ...
    if (reaction.id.remote === groupId && reaction.reaction === "✅") {
      ...
      if (id) {
        ...
        if (
          (senderPhoneNumber && role == "trabajador") ||
          role == "administrador"
        ) {
          await addProductToUserService(
            originalSenderPhoneNumber,
            newProductAddUser
          );
          // mensaje de confimacion 
          await client.sendMessage(
            originalSenderPhoneNumber + "@c.us",
            confirmationMessage
          );
          ...
          // Verificar si el usuario tiene autoGeneratedPassword en false
          const originalUser = await getUserByIdService(originalSenderPhoneNumber);
          if (originalUser.autoGeneratedPassword === true) {
            const credentials = await getUserCredentialsService(originalSenderPhoneNumber);
            const usernameRegex = /^user\d{5}$/; // Expresión regular para verificar el nombre de usuario

            if (usernameRegex.test(credentials.username)) {
              const credentialsMessage = `Tus credenciales son:\nUsuario: ${credentials.username}\nContraseña: ${credentials.password}`;
              await client.sendMessage(
                originalSenderPhoneNumber + "@c.us",
                credentialsMessage
              );
