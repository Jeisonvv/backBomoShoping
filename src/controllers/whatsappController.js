...
const sendMessageController = async (req, res) => {
  ...
  try {
    ...
    if (!lastProduct) {
      return res
        .status(404)
        .json({ error: "No se encontró el último producto guardado." });
    }
    ...
    res
      ...
      .json({
        success: true,
        message: "Producto guardado y mensaje enviado correctamente.",
      });
  } catch (error) {
...
const getQrCodeController = (req, res) => {
  ...
  if (!status.authenticated && status.qrCode) {
    ...
    res
      .status(500)
      .json({ error: "No se pudo obtener el QR en este momento." });
  }
};
...
const sendSaleReportControlle = async (req, res) => {
  try {
    await saleReportWatsappService();
    res.status(200).json({ message: "Reporte de ventas enviado con éxito." });
  } catch (error) {
